...
function loadStatsPage(){
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++){//Outer for loop iterates over each row
		//Change winning team to opponent if they had a higher score
...
function loadPlayersPage(){
	...
	for(i=0; i<players.length; i++){
		aTag = '<p><a href= # onclick=switchPlayer(' + i + ')>' + players[i].name + '</a></p>';
		//aTag.setAttribute('href',"#");
		//aTag.innerText = players[i].name;
		//aTag.onclick = switchPlayer(i);
		text = text + aTag;
...
function switchPlayer(i){
	...
	document.getElementById("avg_p_yards").innerHTML = (players[i].pass_yards/players[i].games_played).toFixed(2);
	document.getElementById("avg_r_yards").innerHTML = (players[i].rushing_yards/players[i].games_played).toFixed(2);
	document.getElementById("avg_rec_yards").innerHTML = (players[i].receiving_yards/players[i].games_played).toFixed(2);
